...
public protocol GarmentRepositoryProtocol: Sendable {
    ...
    /// Find garments by visual similarity using pre-computed embedding
    func findByVisualSimilarity(embedding: [Float], topK: Int, sourceFilter: [GarmentSource]) async throws -> [VisualSearchResult]
    
    /// Update garment embedding (768-dim)
    func updateEmbedding(garmentId: UUID, embedding: [Float]) async throws -> Garment
}
...
public struct GarmentFilterCriteria: Hashable, Sendable {
    ...
    public var sources: [GarmentSource]? // v3.5: Filter by source
    ...
    public init(
        ...
        sources: [GarmentSource]? = nil, // v3.5
...
public enum GarmentSortOption: String, Hashable, Sendable {
    ...
    case brand = "brand"
    case visualSimilarity = "visual_similarity" // v3.5: Sort by embedding similarity
}
